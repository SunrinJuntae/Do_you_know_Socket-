...
#include <string.h>
#include <stdbool.h>
#include <Ws2tcpip.h>			//inet_pton 포함 헤더
...
int main() {
	...
	memset(&tAddr, 0, sizeof(SOCKADDR_IN));			   //구조체 0으로 초기화
	tAddr.sin_family = AF_INET;
	tAddr.sin_port = htons(PORT);               
	inet_pton(AF_INET, &SERVER_IP, &(tAddr.sin_addr.s_addr));	//서버 ip주소
	
	connect(hSocket, (SOCKADDR*)& tAddr, sizeof(tAddr));       //지정된 소켓에 연결(=bind)

	chatting(hSocket);                                           //채팅 시작
...
void chatting(SOCKET hSocket) {                //채팅 부분
	int nRcv;
